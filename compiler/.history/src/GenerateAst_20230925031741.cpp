...
string GenerateAst::addPointer(string s){
    if (s == "Expr" || s == "Unary" || s == "Binary" || s == "Literal" || s == "Grouping") return s + "*";
    return s;
}
...
void GenerateAst::defineTypeCpp(ofstream& file, string baseName, string className, string fieldList){
    ...
    vector<array<string, 2>> field_map = {};
    ...
    while((pos = temp.find(",")) != -1){
        ...
        field_map.push_back({field_name, field_type});
    ...
    for (auto const& x : field_map){
        file << addPointer(x[1]) + " " + x[0] + (counter < num_fields - 1 ? ", " : "");
        counter++;
    }
    file << ") : ";
    counter = 0;
    for (auto const& x : field_map){
        file << x[0] + "(" + x[0] + ")" + (counter < num_fields - 1 ? ", " : "");
        counter++;
    }
    file << " {}" << endl;
    ...
    file << "\treturn visitor->visit" + className + "(this);" << endl;
    file << "}" << endl;
    file << endl;
}
...
void GenerateAst::generateH(string outputDir, string baseName, vector<array<string, 2>> class_fieldsmap){
    ...
    file << "class Binary;" << endl;
    file << "class Grouping;" << endl;
    file << "class Literal;" << endl;
    file << "class Unary;" << endl;
    file << endl;
...
void GenerateAst::generateVisitor(ofstream& file, vector<array<string, 2>> class_fieldmap){
    ...
    for (auto const& kp : class_fieldmap){
        ...
        file << "\tvirtual string visit" + className + "(" + addPointer(className) + " " + toLower(className) + ") = 0;" << endl;
    }
    file << "};" << endl;
}
