...
void GenerateAst::defineType(ofstream& file, string baseName, string className, string fieldList){
    ...
    cout << temp << endl;
    field_type = temp.substr(0, field.find(" "));
    field_name = temp.substr(field.find(" ") + 1);
    field_map.insert({field_type, field_name});

    // Member variables
    for (auto const& x : field_map){
        cout << "~" + addPointer(x.first) + "~ ~" + x.second + "~" + endl;
    }
    ...
    int count = 0;
    for (auto const& x : field_map){
        file << addPointer(x.first) + " " + x.second + (count < num_fields - 1 ? ", " : "");
        count++;
    }
    file << ") : ";
    count = 0;
    for (auto const& x : field_map){
        file << x.second + "(" + x.second + ")" + (count < num_fields - 1 ? ", " : "");
        count++;
    }
    file << " {}" << endl;
    file << "};" << endl;
